...
/*
Funcion de recompensa para el agente inteligente
Se basa en el numero total de paquetes recibido por el nodo destino 
*/
...
//funcion utilizada para modificar parametros de los nodos segun las acciones
// del agente inteligente
void SetNodeParams (Ptr<Node> node, double txpower, double txgain)
{
  Ptr<NetDevice> device = node->GetDevice (0);
  Ptr<WifiNetDevice> wifiDevice = DynamicCast<WifiNetDevice> (device);
  Ptr<YansWifiPhy> phy = DynamicCast<YansWifiPhy>(wifiDevice->GetPhy());
  //Establecer la potencia de envio y recepcion del nodo 
  phy->SetTxPowerStart(txpower);
  phy->SetTxPowerEnd(txpower);
  //Establecer la ganancia de envio y recepcion del nodo
...
/*
Ejecutar las acciones del agente inteligente
*/
bool MyExecuteActions(Ptr<OpenGymDataContainer> action)
{
  ...
  for (uint32_t i=0; i<nodeNum; i++)
  {
    ...
    //usar el vector de acciones para establecer los parametros de cada nodo
    SetNodeParams(node,txPower, txPower*2);
  } 
